# Mappings from CUIs to specific sources (SABs) - aim is to pick some relevant SABs

downloaded umls-2022AB-mrconso.zip - https://uts.nlm.nih.gov/uts/
- release documentation - https://www.nlm.nih.gov/research/umls/knowledge_sources/metathesaurus/release/index.html 
- source abbrevation and term type - https://www.nlm.nih.gov/research/umls/knowledge_sources/metathesaurus/release/precedence_suppressibility.html
- the SAB names are here https://www.nlm.nih.gov/research/umls/sourcereleasedocs/index.html


# Meta-data processing from the UMLS data files
...
# Create selected subfiles from the massive MRCONSO file for a handful of selected sources (SABs)

however we are using the entity linker in Scispasy which maps onto "all" of UMLS and returns CUIs and we will also tag these onto the papers in addition to the selected SABs (which will have their own attributes)

download - requires NIH login
https://download.nlm.nih.gov/umls/kss/2022AB/umls-2022AB-mrconso.zip

creating a file called umlsSelect.txt

```
zgrep \|MSH\| umls-2022AB-mrconso.zip > cui2MSH.txt
zgrep \|HPO\| umls-2022AB-mrconso.zip > cui2HPO.txt
zgrep \|HGNC\| umls-2022AB-mrconso.zip > cui2HGNC.txt
zgrep \|DRUGBANK\| umls-2022AB-mrconso.zip > cui2DRUGBANK.txt
zgrep \|RXNORM\| umls-2022AB-mrconso.zip > cui2RXNORM.txt
zgrep \|NCI\| umls-2022AB-mrconso.zip > cui2NCI.txt
cat cui* > umlsSelect.txt
```

files created - cui2MSH.txt cui2HPO.txt, cui2HGNC.txt, cui2DRUGBANK.txt, cui2RXNORM.txt, cui2NCI.txt, umlsSelect.txt

# Files also available in BOX
Alternatively, download all files from box folder linked in readme.md.
